...
int getFileVersion(char *fileName, int version, int client_sock)
{
    ...
    snprintf(versionedPath, sizeof(versionedPath), "%s.v%d", basePath, version);
    ...
    if (fd < 0)
    {
        ...
        snprintf(response, sizeof(response), "ERROR version not found\n");
        send(client_sock, response, strlen(response), 0);
    ...
    close(fd);
    printf("Sent %ld bytes from %s (v%d)\n", (long)total_sent, basePath, version);
